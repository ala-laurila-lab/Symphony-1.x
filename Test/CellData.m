classdef CellData < handle
    ...
    methods
        ...
        function [spCount, xvals,varargout] = getPSTH(obj, epochInd, binWidth, streamName, varargin)
            if nargin < 5
                smoothingWindow = 0;%in sec
            else
                smoothingWindow = varargin{1};
            end
            if nargin < 4
                streamName = 'Amplifier_Ch1';
            end
            ...
            %smoothing
            if smoothingWindow
                smoothingWindow_pts = round(smoothingWindow/binWidth ); %for psth
                %counts = filter(ones(1,smoothingWindow_pts)/smoothingWindow_pts,1,counts,[],2);
                w = gausswin(smoothingWindow_pts);
                w = w/sum(w);
                spCount_smth = conv(spCount,w,'same');
                %spCount_smth = filter(w./sum(w),1,spCount,[],2);
            end
            %convert to Hz
